...
Source: .planning/REQUIREMENTS.md (milestone closeout snapshot)
...
- [x] **RUN-01**: User can run a result-returning query without the backend preloading the full result set before the first page is shown in the UI
- [x] **RUN-02**: User can load additional result pages through a supported paging path that preserves query context and clearly surfaces when paging is unsafe or unavailable
- [x] **RUN-03**: User can cancel long-running query and export operations without leaving stuck UI state or orphaned backend task state
- [x] **RUN-04**: User can export current page, loaded rows, or full result through registered runtime commands with explicit limits and warnings
- [x] **RUN-05**: User can work against PostgreSQL schemas beyond `public` by selecting or persisting an active/default schema
