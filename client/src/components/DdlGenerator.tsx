...
export function DdlGenerator({ fileId, sheetName }: DdlGeneratorProps) {
  ...
  const { data: tableInfo } = useTableInfo(fileId, sheetName);
    ...
    generate(
      { tableInfo, dialect },
      {
        onSuccess: (data) => {
          ...
          toast({
            title: "DDL Generated",
            description: `Successfully generated ${dialect.toUpperCase()} DDL script.`,
          });
        },
        onError: (error) => {
          toast({
    ...
    <div className="flex flex-col h-full bg-background border-l border-border">
      <div className="p-4 border-b border-border bg-card/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">DDL Output</h3>
        </div>
      ...
      <div className="flex-1 overflow-hidden relative group bg-slate-950">
        {!generatedDdl ? (
          ...
          <div className="absolute inset-0 overflow-auto custom-scrollbar">
            <pre className="p-6 font-mono text-sm text-slate-200 leading-relaxed selection:bg-primary/30">
          ...
          {generatedDdl && (
            ...
            >
              <Button
                ...
                className="shadow-lg bg-white/10 hover:bg-white/20 text-white border-none backdrop-blur-sm"
              >
